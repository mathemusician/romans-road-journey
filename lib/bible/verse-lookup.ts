...
const verseCache = new Map<string, BibleVerse | BibleVerse[]>();
let bibleData: BibleVerse[] | null = null;

// Load Bible data (lazy loaded)
function loadBibleData(): BibleVerse[] {
  ...
  try {
    ...
    bibleData = JSON.parse(decompressed.toString());
    return bibleData!;
  } catch (error) {
    ...
    bibleData = JSON.parse(fs.readFileSync(uncompressedPath, 'utf8'));
    return bibleData!;
  }
}
...
export function getVerse(reference: string): BibleVerse | null {
  ...
  const verses = loadBibleData();
  const verse = verses.find(
    v => v.book === parsed.book && 
...
// Get verse range
export function getVerseRange(reference: string): BibleVerse[] {
  // Check cache
  if (verseCache.has(reference)) {
    const cached = verseCache.get(reference);
...
export function getChapter(book: string, chapter: number): BibleVerse[] {
  ...
  const result = verses.filter(
    v => v.book === normalizedBook && v.chapter === chapter
  );
